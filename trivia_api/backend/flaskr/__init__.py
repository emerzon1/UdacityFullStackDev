...
def create_app(test_config=None):
  ...
  '''
    def question_get_return(page, cID=None, sTerm=None):
        if cID:
            questions = Question.query.filter(Question.category == cID).paginate(
                page=page, max_per_page=QUESTIONS_PER_PAGE)
            ...
            questions = Question.query.filter(func.lower(Question.question).contains(
                sTerm.lower())).paginate(max_per_page=QUESTIONS_PER_PAGE, page=page)
            category_type = ' '
        else:
    ...
    def add_question():
        ...
        if ((data.get('question') == '') or (data.get('answer') == '') or (data.get('category') == '') or (data.get('difficulty') == '')):
            abort(422)
        try:
            q = Question(question=data.get('question', ''), answer=data.get(
    ...
    def play_quiz():
        ...
        if not quiz_cat:
            abort(422)
        question = Question.query.filter(Question.category == quiz_cat.get('id')).filter(
            Question.id.notin_(prev_list)).order_by(func.random()).limit(1).all()
        if question:
